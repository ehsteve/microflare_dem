PRO aia_hsi_dem_analysis_manual_test, DIR = dir, FILESET = fileset, FORCE_TABLE = force_table, aia_only=aia_only,epstein=epstein,flux_=flux_,pixels=pixels,n=n
...
area = aia_teem_pixel_area(file_list[0,0]) * (4^2)
...
flare_area=0;aia_pixel_area*num_aia_pixels
;stop
...
;IF NOT keyword_set(flux_) THEN BEGIN
;flux_ = fltarr(nwave)
;ENDIF
...
IF keyword_set(aia_only) THEN BEGIN
	...
	IF keyword_set(epstein) THEN BEGIN
		SAVE,aia_fit_results,filename='aia_fit_results_epstein_manual.sav',/verbose
	ENDIF ELSE BEGIN
		SAVE,aia_fit_results,filename='aia_fit_results_manual.sav',/verbose
	ENDELSE
ENDIF ELSE BEGIN
	...
	SAVE,aia_hsi_fit_results,filename='aia_hsi_fit_results_manual.sav',/verbose
ENDELSE
